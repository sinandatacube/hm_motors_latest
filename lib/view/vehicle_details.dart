...
import 'package:hm_motors/controller/dynamic_link_handler.dart';
...
import 'package:hm_motors/view/custom%20widget/favourite_button.dart';
    ...
    return FutureBuilder(
        ...
        builder: (context, AsyncSnapshot snapshot) {
          ...
          if (snapshot.hasData) {
            ...
            return FutureBuilder(
                future:
                    VehicleDetailsController().loadDetails(widget.purchaseId),
                // future: ApiController().fetchvehicleDetails(widget.purchaseId),
                builder: (context, AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    log(snapshot.data.toString(), name: "snapshot");
                    List images2 = snapshot.data['details']["fullImages"] ?? [];
                    images2.add({"Car_Image": widget.thumbImage});
                    VehicleDetailsModel data = VehicleDetailsModel.fromJson(
                        snapshot.data['details']["result"]);
                    // if (wishlistItems.contains(data.id)) {
                    //   isFav = true;
                    // }
                    log(snapshot.data['intrest_status'], name: 'status');
                    if (snapshot.data['intrest_status'] == "true") {
                      isInterested = true;
                    } else {
                      isInterested = false;
                    }
                    for (var element in wish.cars) {
                      if (element.id == widget.purchaseId &&
                          element.status == "1") {
                        isFav = true;
      ...
      child: Row(
        ...
        children: [
          Expanded(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                  backgroundColor: isInterested ? Colors.green : mainColor,
          ...
          FavouriteButton(
              isFav: isFav, purchaseId: widget.purchaseId, wishId: wishId)
          // Card(
          //   elevation: 5,
          //   color: Colors.white,
          //   shape: const CircleBorder(),
